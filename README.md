...
HWIFileDownload simplifies file download with `NSURLSession` on iOS. Parallel file download can be controlled individually with the complete set of possible operations: start, cancel, pause, resume. Single file and total download progress is reported with `NSProgress`.
